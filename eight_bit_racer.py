...
class EightBitRacerController(object):
    def __init__(self):
        self.adb = adb.ADB()
        self.adb.open_shell()
        self.end_signal = Image.open("assets/8-bit-racing/240x320/end_signal.png")
        self.num_actions = 3
    ...
    def right(self):
        ...
        # self.adb.shell_interactive("input tap 1000 2500")
        return

    def left(self):
        self.adb.shell_interactive("input tap 15 290")
        # self.adb.shell_interactive("input tap 400 2500")
        return
    ...
    def _are_screens_similar(self, screen1, screen2):
        ...
        rms = math.sqrt(
            sum(map(lambda h, i: h * (i**2), hist, range(256)))
    ...
    def restart_game(self):
        ...
        # self.adb.shell_interactive("input tap 700 1400")
        # Play Again
        self.adb.shell_interactive("input tap 60 215")
        # self.adb.shell_interactive("input tap 700 1600")
        # self.adb.shell_interactive("input tap 120 155")
        self.score_zone = None
...
def save_sars(
    ...
    action_file = open("screens/%d-%d-action.txt" % (episode_num, counter), "w")
    if q_sa is not None:
...
def run_episode(train, controller, experiences, model, episode_num, epsilon):
    ...
    while not controller.is_gameover(state):
        ...
        if len(current_history) == 4:
            combined = combine_history(current_history).reshape((1, 4, WIDTH, HEIGHT))
            ...
            if len(experiences) >= BATCH_SIZE:
                ...
                for j, sample in enumerate(samples):
                    ...
                    if is_terminal:
                        ...
                        targets[j, action] = reward + DISCOUNT_FACTOR * np.max(
                            model.predict(s2)
                        )
...
def main(args):
    ...
    if args.model_file is not None:
        if os.path.exists(args.model_file):
            ...
            raise Exception("Specified a non-existent model file: " + args.model_file)
    else:
    ...
    for episode_num in range(NUM_EPISODES):
        ...
        score, num_frames = run_episode(
            args.train, controller, experiences, model, episode_num, epsilon
        )
        ...
        if episode_num % CHECKPOINT_INTERVAL == 0:
            ...
            model.save(
                DEFAULT_MODEL_FILE if args.model_file is None else args.model_file
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to train the agent or just run inference on the saved model",
    )
    parser.add_argument(
        "--model_file",
        type=str,
        default=None,
        help="If specified, resumes from a stored model file. Otherwise creates a model file at default model.h5",
    )
    parser.add_argument(
        "--epsilon",
        type=float,
        default=START_EPSILON,
        help="Starting epsilon value for epsilon greedy training. If train is False, this does nothing.",
    )
