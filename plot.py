...
def plotComparison():
    ...
    N = 25
    ind = np.arange(N)
    scoresB = [float(s) for s in open('scores-random.txt', 'r').read().splitlines()]
    scoresQ = [float(s) for s in open('scores-best.txt', 'r').read().splitlines()]
    scoresO = [float(s) for s in open('scores-oracle.txt', 'r').read().splitlines()]
...
if __name__ == "__main__":
    ...
    plotScoreAverages(scores, averageInterval=500)
    plotComparison()
